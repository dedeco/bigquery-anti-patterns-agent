...
class LLMQueryAnalyzer:
    ...
    def analyze_query(self, query_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            data = {
                ...
                # System prompt as separate parameter, not a message
                "messages": [
            ...
            return {
                "analysis": {
                    ...
                    "multiple_with_clauses": query_text.upper().count(
                        "WITH") > 3,
                ...
                "explanations": {
                    ...
                    "multiple_with_clauses": "Too many WITH clauses found." if query_text.upper().count(
                        "WITH") > 3 else "",
                    "subquery_with_aggregation": "Subqueries with aggregation can be inefficient." if any(
                        func in query_text.upper() for func in
                        ["COUNT(", "SUM(", "AVG(", "MIN(", "MAX("]) else "",
                    "subquery_with_distinct": "DISTINCT in subqueries can be expensive." if "DISTINCT" in query_text.upper() else "",
                    "too_many_joins": "Having many joins can lead to performance issues." if query_text.upper().count(
                        "JOIN") > 3 else "",
    ...
    def optimize_query(self, query_text: str, analysis: Dict[str, Any]) -> str:
        ...
        try:
            ...
            import re
            sql_match = re.search(r'```sql\s*(.*?)\s*```', optimized_query,
                                  re.DOTALL)
            if sql_match:
            ...
            if analysis["analysis"].get("select_star", False):
                optimized_query = optimized_query.replace("SELECT *",
                                                          "SELECT id, name, value, timestamp /* Select only necessary columns */")

            if analysis["analysis"].get("order_by_without_limit",
                                        False) and "LIMIT" not in optimized_query:
